...
* Rutas en `app.py`: `GET/POST /monitoring` (alta), `GET /monitoring/lista` (lista de todos los dominios registrados), `GET /monitoring/<access_token>` (dashboard de un dominio), `POST /webhooks/dmarc-aggregate/<secret>` (recibe el JSON de parsedmarc; `<secret>` debe matchear `DMARC_WEBHOOK_SECRET` o responde 404 en vez de 401, para no delatar que la ruta existe).
* **Decisión explícita sobre `/monitoring/lista`**: es pública y sin protección — cualquiera puede ver todos los dominios registrados y entrar a cualquier dashboard sin necesitar su `access_token`. Se preguntó directamente al usuario (proteger con contraseña de admin vs. dejarla pública) y eligió dejarla pública. Esto en la práctica anula la privacidad que el `access_token` daba por sí solo (ver nota abajo) — si en algún momento se quiere volver a restringir el acceso por dominio, hay que revisar primero esta ruta, no solo el dashboard.
