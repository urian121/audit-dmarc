...
* **SPF con detección de proveedor** (`detect_mail_provider()`, `KNOWN_MX_PROVIDERS` en `checkdmarc_service.py`): si el dominio no tiene SPF, nunca se sugiere un valor final (podría rechazar correo legítimo si no conocemos todos los remitentes) — pero sí se consultan los MX y, si el hostname matchea un proveedor conocido (Google/Microsoft/Zoho/etc., heurística por texto, no oficial), se muestra un `include:` de partida junto con una nota explicando que hay que sumar también herramientas de marketing/transaccionales y servidores propios antes de publicarlo. Si no matchea ningún proveedor conocido, sólo se muestra la guía genérica.
